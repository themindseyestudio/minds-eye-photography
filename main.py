...
def get_portfolio():
    ...
    try:
        # Scan photography-assets directory for actual images
        portfolio_data = []
        
        # Get images from photography-assets directory
        if os.path.exists(PHOTOGRAPHY_ASSETS_DIR):
            for filename in os.listdir(PHOTOGRAPHY_ASSETS_DIR):
                if filename.lower().endswith(('.jpg', '.jpeg', '.png', '.gif')):
                    ...
                    entry = {
                        "id": image_id,
                        "title": title,
                        "description": f"Photography by Mind's Eye Studio",
                        "image": filename,
        ...
        # Sort by filename (most recent uploads last)
        portfolio_data.sort(key=lambda x: x['image'])
